...
interface IIdleToken {
  ...
  //not included in the interface in idle docs, but it states that it is supported so i'm gonna define it here
  // so that i don't get a compile error
  function balanceOf(address who) external view returns (uint256);
  
  // don't really need this function, but it's here for completeness
